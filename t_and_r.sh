...
if [[ "$use_saved" != "y" ]]; then
    # Prompt the user for the Python file to transfer and run
    read -p "Enter the name of the Python file to transfer and run: " PYTHON_FILE
...
# Print the file path and then run the Python file on the Raspberry Pi
ssh "$PI_USER@$PI_HOST" "echo $PI_PATH/$FILENAME && python3 $PI_PATH/$FILENAME"
